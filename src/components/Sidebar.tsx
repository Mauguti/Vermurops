...
import {
  ...
  DollarSign, Files, Settings as Settings2, Calculator, Tag, Bell,
} from 'lucide-react';
...
export default function Sidebar({ currentView, onChangeView }: SidebarProps) {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className="hidden md:flex flex-col w-[240px] bg-surface-dark border-r border-surface-border min-h-screen text-text-inverse shrink-0 fixed z-20">
        <div className="h-[60px] px-[24px] flex items-center border-b border-surface-border shrink-0">
          <div className="flex items-center w-full">
            <div className="bg-white rounded py-2 px-3 flex items-center justify-center w-full">
              <img
        ...
        <nav className="flex-1 mt-6 px-[12px]">
          <div className="text-[11px] font-semibold text-text-inverse-muted uppercase tracking-[0.05em] mb-3 px-[12px]">
            Plataforma
          </div>
                ...
                <li key={item.id}>
                  <button
                    ...
                    className={`w-full flex items-center px-[12px] py-[8px] text-[14px] rounded-md transition-colors ${
                      isActive
                        ? 'bg-surface-elevated text-brand font-medium shadow-sm'
                        : 'text-text-inverse-muted hover:bg-surface-elevated/50 hover:text-text-inverse'
                    }`}
                  >
                    {item.icon}
                    {item.label}
                    {isNotif && conteoNoLeidas > 0 && (
                      <span className="ml-auto min-w-[18px] h-[18px] bg-[#E11D48] text-white text-[10px] font-black rounded-full flex items-center justify-center px-[3px]">
        ...
        <div className="p-[24px] text-[12px] text-text-inverse-muted">
          VermurOps v2.4
        </div>
      </div>

      {/* Mobile bottom nav */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-surface-dark border-t border-surface-border z-50">
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center py-3 px-2 shrink-0 relative ${
                  currentView === item.id ? 'text-brand' : 'text-text-inverse-muted'
                }`}
              >
                <div className="mb-1 relative">
                  {item.icon}
                  {isNotif && conteoNoLeidas > 0 && (
                    <span className="absolute -top-[4px] -right-[6px] min-w-[14px] h-[14px] bg-[#E11D48] text-white text-[9px] font-black rounded-full flex items-center justify-center px-[2px]">
                      {conteoNoLeidas > 9 ? '9+' : conteoNoLeidas}
                    </span>
                  )}
                </div>
